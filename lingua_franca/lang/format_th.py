...
from lingua_franca.lang.common_data_th import _NUM_STRING_TH, \
    _FRACTION_STRING_TH, _LONG_SCALE_TH, _SHORT_SCALE_TH, _SHORT_ORDINAL_TH, _LONG_ORDINAL_TH


def nice_number_th(number, speech, denominators=range(1, 21)):
    ...
    if num == 0:
        return str(whole)
    den_str = _FRACTION_STRING_TH[den]
    if whole == 0:
        if num == 1:
...
def pronounce_number_en(num, places=2, short_scale=True, scientific=False,
    ...
    if short_scale:
        ...
        number_names.update(_SHORT_SCALE_TH)
    else:
        number_names = _NUM_STRING_TH.copy()
        number_names.update(_LONG_SCALE_TH)
    ...
    if short_scale:
        hundreds = [_SHORT_SCALE_TH[n] for n in _SHORT_SCALE_TH.keys()]
    else:
        hundreds = [_LONG_SCALE_TH[n] for n in _LONG_SCALE_TH.keys()]
    ...
    if num in number_names and not ordinals:
        ...
        def _sub_thousand(n, ordinals=False):
            assert 0 <= n <= 999
            if n in _SHORT_ORDINAL_TH and ordinals:
                return _SHORT_ORDINAL_TH[n]
            if n <= 19:
                return digits[n]
            elif n <= 99:
                q, r = divmod(n, 10)
            ...
            for i, z in enumerate(_split_by(n, 1000)):
                ...
                if i:
                    ...
                    if ordi:
                        ...
                        if i * 1000 in _SHORT_ORDINAL_TH:
                            if z == 1:
                                number = _SHORT_ORDINAL_TH[i * 1000]
                            else:
                                number += _SHORT_ORDINAL_TH[i * 1000]
                        else:
                            if n not in _SHORT_SCALE_TH:
                                num = int("1" + "0" * (len(str(n)) - 2))

                                number += _SHORT_SCALE_TH[num] + "th"
                            else:
                                number = _SHORT_SCALE_TH[n] + "th"
                    else:
                        number += hundreds[i]
                res.append(number)
                ordi = False
        ...
        def _long_scale(n):
            ...
            for i, z in enumerate(_split_by(n, 1000000)):
                ...
                if i:
                    ...
                    if ordi:
                        if i * 1000000 in _LONG_ORDINAL_TH:
                            if z == 1:
                                number = _LONG_ORDINAL_TH[
                                    (i + 1) * 1000000]
                            else:
                                number += _LONG_ORDINAL_TH[
                                    (i + 1) * 1000000]
                        else:
                            if n not in _LONG_SCALE_TH:
                                num = int("1" + "0" * (len(str(n)) - 2))

                                number += " " + _LONG_SCALE_TH[
                                    num] + "th"
                            else:
                                number = " " + _LONG_SCALE_TH[n] + "th"
                    else:
...
def nice_time_en(dt, speech=True, use_24hour=False, use_ampm=False):
    ...
    if use_24hour:
        ...
        if string[0] == '0':
            speak += pronounce_number_th(int(string[0])) + " "
            speak += pronounce_number_th(int(string[1]))
        else:
            speak = pronounce_number_th(int(string[0:2]))
        ...
        if string[3:5] == '00':
            ...
            if string[3] == '0':
                speak += pronounce_number_th(0) + " "
                speak += pronounce_number_th(int(string[4]))
            else:
                speak += pronounce_number_th(int(string[3:5]))
        return speak
    else:
        ...
        if dt.minute == 15:
            speak = "quarter past " + pronounce_number_th(hour)
        elif dt.minute == 30:
            speak = "half past " + pronounce_number_th(hour)
        elif dt.minute == 45:
            next_hour = (dt.hour + 1) % 12 or 12
            speak = "quarter to " + pronounce_number_th(next_hour)
        else:
            speak = pronounce_number_th(hour)
            ...
            if dt.minute == 0:
                ...
                if dt.minute < 10:
                    speak += " oh"
                speak += " " + pronounce_number_th(dt.minute)
