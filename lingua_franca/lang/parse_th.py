...
from lingua_franca.lang.common_data_th import _ARTICLES_TH, _NUM_STRING_TH, \
    _LONG_ORDINAL_TH, _LONG_SCALE_TH, _SHORT_SCALE_TH, _SHORT_ORDINAL_TH
...
_MULTIPLIES_LONG_SCALE_TH = set(_LONG_SCALE_TH.values()) | \
    generate_plurals_th(_LONG_SCALE_TH.values())

_MULTIPLIES_SHORT_SCALE_TH = set(_SHORT_SCALE_TH.values()) | \
    generate_plurals_th(_SHORT_SCALE_TH.values())
...
_STRING_NUM_TH = invert_dict(_NUM_STRING_TH)
_STRING_NUM_TH.update(generate_plurals_th(_STRING_NUM_TH))
_STRING_NUM_TH.update({
    "half": 0.5,
    "halves": 0.5,
    "couple": 2
})

_STRING_SHORT_ORDINAL_TH = invert_dict(_SHORT_ORDINAL_TH)
_STRING_LONG_ORDINAL_TH = invert_dict(_LONG_ORDINAL_TH)


def _convert_words_to_numbers_th(text, short_scale=True, ordinals=False):
    """
    ...
    numbers_to_replace = \
        _extract_numbers_with_text_th(tokens, short_scale, ordinals)
...
def _extract_numbers_with_text_th(tokens, short_scale=True,
    ...
    while True:
        to_replace = \
            _extract_number_with_text_th(tokens, short_scale,
...
def _extract_number_with_text_th(tokens, short_scale=True,
                                 ordinals=False, fractional_numbers=True):
    """
    ...
    """
    number, tokens = \
        _extract_number_with_text_th_helper(tokens, short_scale,
                                            ordinals, fractional_numbers)
    while tokens and tokens[0].word in _ARTICLES_TH:
        tokens.pop(0)
    return ReplaceableNumber(number, tokens)


def _extract_number_with_text_th_helper(tokens,
                                        short_scale=True, ordinals=False,
                                        fractional_numbers=True):
    """
    Helper for _extract_number_with_text_th.
    ...
    if fractional_numbers:
        fraction, fraction_text = \
            _extract_fraction_with_text_th(tokens, short_scale, ordinals)
        if fraction:
            return fraction, fraction_text

        decimal, decimal_text = \
            _extract_decimal_with_text_th(tokens, short_scale, ordinals)
        if decimal:
            return decimal, decimal_text

    return _extract_whole_number_with_text_th(tokens, short_scale, ordinals)


def _extract_fraction_with_text_th(tokens, short_scale, ordinals):
    """
    ...
    for c in _FRACTION_MARKER:
        ...
        if len(partitions) == 3:
            numbers1 = \
                _extract_numbers_with_text_th(partitions[0], short_scale,
                                              ordinals, fractional_numbers=False)
            numbers2 = \
                _extract_numbers_with_text_th(partitions[2], short_scale,
...
def _extract_decimal_with_text_th(tokens, short_scale, ordinals):
    """
    ...
    Notes:
        While this is a helper for extractnumber_th, it also depends on
        extractnumber_th, to parse out the components of the decimal.
...
def _extract_whole_number_with_text_th(tokens, short_scale, ordinals):
    """
    ...
    for idx, token in enumerate(tokens):
        ...
        word = token.word
        if word in _ARTICLES_TH or word in _NEGATIVES:
        ...
        if word not in string_num_scale and \
                word not in _STRING_NUM_TH and \
                word not in _SUMS and \
                word not in multiplies and \
                not (ordinals and word in string_num_ordinal) and \
                not is_numeric(word) and \
                not isFractional_th(word, short_scale=short_scale) and \
                not look_for_fractions(word.split('/')):
            words_only = [token.word for token in number_words]
            if number_words and not all([w in _ARTICLES_TH |
                ...
                and prev_word not in _ARTICLES_TH:
            number_words = [token]
        ...
        if word in _STRING_NUM_TH:
            val = _STRING_NUM_TH.get(word)
            current_val = val
        ...
        # half cup
        if val is False:
            val = isFractional_th(word, short_scale=short_scale)
            current_val = val

        # 2 fifths
        if not ordinals:
            next_val = isFractional_th(next_word, short_scale=short_scale)
            if next_val:
                if not val:
                    val = 1
                val = val * next_val
...
def _initialize_number_data(short_scale):
    ...
    """
    multiplies = _MULTIPLIES_SHORT_SCALE_TH if short_scale \
        else _MULTIPLIES_LONG_SCALE_TH

    string_num_ordinal_th = _STRING_SHORT_ORDINAL_TH if short_scale \
        else _STRING_LONG_ORDINAL_TH

    string_num_scale_th = _SHORT_SCALE_TH if short_scale else _LONG_SCALE_TH
    string_num_scale_th = invert_dict(string_num_scale_th)
    string_num_scale_th.update(generate_plurals_th(string_num_scale_th))
    return multiplies, string_num_ordinal_th, string_num_scale_th


def extractnumber_th(text, short_scale=True, ordinals=False):
    """
    ...
    """
    return _extract_number_with_text_th(tokenize(text.lower()),
                                        short_scale, ordinals).value


def extract_duration_th(text):
    """
...
def extract_datetime_th(string, dateNow, default_time):
...
def isFractional_en(input_str, short_scale=True):
    ...
    if short_scale:
        for num in _SHORT_ORDINAL_TH:
            if num > 2:
                fracts[_SHORT_ORDINAL_TH[num]] = num
    else:
        for num in _LONG_ORDINAL_TH:
            if num > 2:
                fracts[_LONG_ORDINAL_TH[num]] = num
...
def extract_numbers_en(text, short_scale=True, ordinals=False):
    ...
    """
    results = _extract_numbers_with_text_th(tokenize(text),
